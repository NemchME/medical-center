...
export function MedCardPage() {
  ...
  const completedVisits = useMemo(
    () =>
      appointments
        .filter((a) => a.status === 'completed' && a.visit)
        .sort(
  ...
  const columns: Column<Appointment>[] = [
    ...
    },
    {
      key: 'doctorName',
      header: 'Врач',
      render: (row) => <span className="text-gray-900">{row.doctor?.fullName ?? '—'}</span>,
    },
    {
      key: 'specialization',
    ...
    },
    {
      ...
      header: 'Диагноз',
      render: (row) => (
        <span className="text-gray-700">{row.visit?.diagnosis || '\u2014'}</span>
      ),
    },
    {
      key: 'status',
  ...
  if (isLoading) {
    return <div className="text-gray-500 text-center py-12">Загрузка карты…</div>;
  }

  if (completedVisits.length === 0) {
    return (
      <div className="space-y-6">
        <h2 className="text-2xl font-bold text-gray-900">Медицинская карта</h2>
        <EmptyState
          icon={ClipboardList}
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Медицинская карта</h2>
        <p className="mt-1 text-sm text-gray-500">Протоколы завершённых визитов</p>
      </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                {apt.visit.notes && (
                  <div className="rounded-xl bg-white p-4 border border-gray-100">
                    ...
                    <p className="text-sm text-gray-600 italic">{apt.visit.notes}</p>
                  </div>
                )}
              </div>

              {(apt.visit.prescriptions?.length ?? 0) > 0 && (
                <div>
